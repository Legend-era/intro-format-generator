...
if os.path.exists("college_logo.png"):
    st.markdown(
        "<div style='text-align: center;'><img src='college_logo.png' width='150'></div>",
        unsafe_allow_html=True
    )
...
hobbies_database = [
    ...
    "Karate","Judo","Taekwondo","Kickboxing","Muay Thai","Fencing","Kalaripayattu",
    "Jiu-Jitsu","Rock Climbing","Mountaineering","Trekking","Hiking","Paragliding",
    "Skydiving","Bungee Jumping","Scuba Diving","Singing","Dancing (Classical)",
    "Painting","Photography","Acting / Theatre","Guitar","Piano","Violin","Flute",
    "Tabla","Sitar","Poetry Writing","Story Writing","Film Making","Animation",
    "Blogging","Vlogging","Coding / Programming","Robotics","Gaming (PC)",
    "Cooking","Baking","Gardening","Watching films/movies","Watching series"
]
...
"""
            st.markdown(
                f"<div style='background-color: #e0f7fa; padding: 20px; border-radius: 10px;'>{casual_intro}</div>",
                unsafe_allow_html=True
            )
        else:
...
"""
            st.markdown(
                f"<div style='background-color: #fff3e0; padding: 20px; border-radius: 10px;'>{professional_intro}</div>",
                unsafe_allow_html=True
            )
        else:
...
# --- Footer ---
st.markdown(
    "<hr><p style='text-align:center'>Developed by Mallick S. | Contact: Soumyakanta2005@outlook.com</p>",
    unsafe_allow_html=True
)
